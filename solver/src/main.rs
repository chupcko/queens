...
type Colors = HashMap<Color, Vec<Cordinate>>;
...
impl Game {
  ...
  fn solve(& self) {
    ...
    fn mark_one(board: &mut Board, row: usize, column: usize) -> bool {
      if board[row][column].state == State::Queen {
        return false;
      }
      board[row][column].state = State::Taken;
      true
    }
    ...
    fn mark_all(game: &mut Game, row: usize, column: usize) -> bool {
      for i in 0 .. game.size {
        if
          !mark_one(&mut game.board, row, i) ||
          !mark_one(&mut game.board, i, column)
        {
          return false;
        }
      }
      if
        (
          row > 0 &&
          (
            (column > 0 && !mark_one(&mut game.board, row-1, column-1)) ||
            (column < game.size-1 && !mark_one(&mut game.board, row-1, column+1))
          )
        ) ||
        (
          row < game.size-1 &&
      ...
      let colors = game.colors.get(&game.board[row][column].color);
      let Some(colors) = colors else {
        panic!("WRONG DATA");
      };
      for cordinate in colors {
        if !mark_one(&mut game.board, cordinate.row, cordinate.column) {
          return false;
        }
      }
